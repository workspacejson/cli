...
async function runAudit(repoRoot, config = {}) {
  ...
  const { workspaceJson, workspaceJsonFound, workspaceJsonStale } = await loadWorkspaceJson(resolvedRoot, agentsMd.lastModified, validator);
  const ctx = {
    agentsMd,
    repo,
    config: fullConfig
