// select-cohort.mjs — META-289 §5.2 eligibility + §5.4 seeded ordering.
//
// Reads ONLY the committed universe snapshot. Produces the frozen ranked order
// per stratum. Fixed before any repository is cloned, so §5.6 backfill cannot
// be steered. Reads METADATA ONLY.
import { readFileSync, writeFileSync } from 'node:fs';
import { createHash } from 'node:crypto';

const SEED = 'META-289/OQ-15/source-test-coupdate/v1';          // §5.4
const EXCLUDED = new Set([                                       // §4 anti-leak
  'formatjs/formatjs', 'JamieMason/syncpack', 'polyfy/polylith',           // META-375
  'nteract/hydrogen', 'thepowersgang/rust_os', 'clojure/core.typed',       // META-378
  'hyperledger/fabric', 'scikit-image/scikit-image',
]);

const u = JSON.parse(readFileSync('docs/evidence/meta-289/raw/universe.json', 'utf8'));
...
writeFileSync('docs/evidence/meta-289/raw/ranked-order.json', `${JSON.stringify(out, null, 2)}\n`);
