// classify.mjs — META-289 §6 source classifier, §7 test classifier, §16 B1 rules.
//
// Single shared module. Language-keyed but REPOSITORY-AGNOSTIC: the same rules
// run over every repository in the cohort, with no per-repository branch.
// Imported by phase-a, phase-b and checks so that one definition governs
// classification, denominators and baselines alike (invariant I8).
...
export function isSource(p) {
  ...
  return !isTest(p);            // TEST takes precedence — roles are exclusive
}
...
// One pass of the frozen §16 strip list, in the frozen order.
...
// Mirrored-layout normalization. Applied uniformly to every repository; it
// simply does not fire where the layout is absent.
...
// Frozen ordering: score descending, ties broken by ascending lexical path.
// The tie-break carries no popularity or structural information (§14).
